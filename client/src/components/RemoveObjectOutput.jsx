...
function RemoveObjectOutput({ processedImageUrl, loading, error }) {
      ...
      {loading && (
        <div className="flex flex-1 items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            <Loader className="w-8 h-8 animate-spin text-[#4A7AFF]" />
            <p className="text-sm text-gray-600">Removing object...</p>
          </div>
        </div>
      )}
      ...
      {showPlaceholder && (
        <div className="flex flex-col gap-2 items-center justify-center flex-1 ">
          ...
          <p className="text-gray-400 text-center">
            Upload an image, specify the object to remove, and click "Remove Object" to get started
          </p>
        </div>
      )}
    </div>
